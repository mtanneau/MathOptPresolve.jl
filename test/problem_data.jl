function run_tests_pbdata(T::Type)
    ...
    @testset "Creation" begin
        pb = MathOptPresolve.ProblemData{T}("test")
        ...
        #= 
            ...
            s.t.    0 ⩽ x1 ⩽ ∞
                    1 ⩽ x2 ⩽ ∞ =#
        MathOptPresolve.add_variable!(pb, Int[], T[],     one(T), zero(T), T(Inf), "x1")
        MathOptPresolve.add_variable!(pb, Int[], T[], 2 * one(T),  one(T), T(Inf), "x2")
        ...
        #= 
            ...
            s.t.    -∞ ⩽  -x1 +   x2 ⩽ 1
                    ...
                    0 ⩽ x1 ⩽ ∞
                    1 ⩽ x2 ⩽ ∞ =#
        MathOptPresolve.add_constraint!(pb, [1, 2], T.([-1, 1]), T(-Inf), one(T), "row1")
        MathOptPresolve.add_constraint!(pb, [1, 2], T.([2, -2]), -one(T), zero(T), "row2")
    ...
    @testset "Delete" begin
        ...
        MathOptPresolve.add_variable!(pb, Int[], T[],     one(T), 11 * one(T), 110 * one(T), "x1")
        MathOptPresolve.add_variable!(pb, Int[], T[], 2 * one(T), 22 * one(T), 220 * one(T), "x2")
        MathOptPresolve.add_variable!(pb, Int[], T[], 3 * one(T), 33 * one(T), 330 * one(T), "x3")

        MathOptPresolve.add_constraint!(pb, [1], T.([1]), 1 * one(T), 10 * one(T), "row1")
        MathOptPresolve.add_constraint!(pb, [2], T.([2]), 2 * one(T), 20 * one(T), "row2")
        MathOptPresolve.add_constraint!(pb, [3], T.([3]), 3 * one(T), 30 * one(T), "row3")

        # Delete row 1 and check remaining problem
        MathOptPresolve.delete_constraint!(pb, 1)
        ...
        # Delete variable 2
        MathOptPresolve.delete_variable!(pb, 2)
        @test pb.ncon == 2
        @test pb.nvar == 2
        col1, col3 = pb.acols
