...
function lowerbound_cg!(ps::PresolveData{T}, i::Int, i_index::Vector{Int}, search_range::Int = 5) where {T}
    ...
    for s in set
        if any(a < ceil.(s*a)*l/ceil(l*s))
            continue
        elseif any(a > ceil.(s*a)*l/ceil(l*s))
            #update row
...
function upperbound_cg!(ps::PresolveData{T}, i::Int, nonzero_index::Vector{Int}, search_range::Int = 5) where {T}
end
